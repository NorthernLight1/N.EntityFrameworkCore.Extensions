...
public class TestDbContext : DbContext
{
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        if (Config.IsPostgreSql)
            optionsBuilder.UseNpgsql(Config.GetTestDatabaseConnectionString());
        else
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<ProductWithComplexKey>().Property<Guid>("Key1").HasDefaultValueSql(Config.IsPostgreSql ? "gen_random_uuid()" : "newsequentialid()");
        modelBuilder.Entity<ProductWithComplexKey>().Property<Guid>("Key2").HasDefaultValueSql(Config.IsPostgreSql ? "gen_random_uuid()" : "newsequentialid()");
        modelBuilder.Entity<ProductWithComplexKey>().HasKey(p => new { p.Key3, p.Key4 });
        modelBuilder.Entity<Order>().Property<DateTime>("DbAddedDateTime").HasDefaultValueSql(Config.IsPostgreSql ? "CURRENT_TIMESTAMP" : "getdate()");
        if (Config.IsPostgreSql)
            modelBuilder.Entity<Order>().Property<DateTime>("DbModifiedDateTime").HasDefaultValueSql("CURRENT_TIMESTAMP").ValueGeneratedOnAddOrUpdate();
        else
            modelBuilder.Entity<Order>().Property<DateTime>("DbModifiedDateTime").HasComputedColumnSql("getdate()");
        modelBuilder.Entity<Order>().Property<bool>(p => p.DbActive).HasDefaultValueSql(Config.IsPostgreSql ? "TRUE" : "((1))");
