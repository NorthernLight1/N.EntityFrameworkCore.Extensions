...
internal sealed class SqlBuilder
{
    ...
    internal List<(string Name, DbType DbType, int Size, object Value)> Parameters { get; private set; }
            ...
            if (keyword != null && isWordStart)
            {
                ...
                if (!string.IsNullOrEmpty(curClause))
                {
                    if (curClause == "DECLARE")
                    {
                        ...
                        DbType dbType = (DbType)Enum.Parse(typeof(DbType), dbTypeString, true);
                        ...
                        Parameters.Add((declareParts[0], dbType, size, value));
                    }
                    else
                    {
