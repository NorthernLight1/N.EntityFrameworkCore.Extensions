...
public static class DbContextExtensions
{
    ...
    public static int UpdateFromQuery<T>(this IQueryable<T> queryable, Expression<Func<T, T>> updateExpression, int? commandTimeout = null) where T : class
    {
        ...
        {
            // EF Core couldn't translate the value expression (e.g. Math.Ceiling, Convert.ToString);
            // fall back to a manually-built raw SQL UPDATE with SQLite-specific syntax.
            using var dbTransactionContext = new DbTransactionContext(dbContext, commandTimeout);
            try
            {
                int rowsAffected = ExecuteRawUpdateFallback(queryable, updateExpression, dbContext, commandTimeout);
                dbTransactionContext.Commit();
                return rowsAffected;
            }
            catch (Exception)
    ...
    private static int ExecuteRawUpdateFallback<T>(IQueryable<T> queryable, Expression<Func<T, T>> updateExpression, DbContext dbContext, int? commandTimeout) where T : class
    {
        ...
        var parameters = ParseQueryStringParams(querySql);
        string alias = ExtractTableAlias(querySql);
        string whereClause = ExtractWhereClause(querySql, alias);
        ...
        return dbContext.Database.ExecuteSql(updateSql, parameters?.ToArray(), commandTimeout);
    }
    private static string ExtractTableAlias(string querySql)
    {
    ...
    private static string ExtractWhereClause(string querySql, string alias)
    {
        string sql = string.Join("\n", querySql.Split('\n').Where(l => !l.TrimStart().StartsWith("--")));
    ...
    private static List<SqliteParameter> ParseQueryStringParams(string querySql)
    {
