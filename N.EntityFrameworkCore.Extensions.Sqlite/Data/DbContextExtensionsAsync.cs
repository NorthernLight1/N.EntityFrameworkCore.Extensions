...
public static class DbContextExtensionsAsync
{
    ...
    public static async Task<int> BulkDeleteAsync<T>(this DbContext context, IEnumerable<T> entities, BulkDeleteOptions<T> options, CancellationToken cancellationToken = default)
    {
        ...
        using (var dbTransactionContext = new DbTransactionContext(context, options))
        {
            ...
            try
            {
                ...
                string deleteSql = context.Database.IsSqliteProvider()
                    ? $"DELETE FROM {destinationTableName} AS t WHERE EXISTS (SELECT 1 FROM {stagingTableName} AS s WHERE {joinCondition})"
                    : context.Database.IsPostgreSql()
                        ? $"DELETE FROM {destinationTableName} AS t USING {stagingTableName} AS s WHERE {joinCondition}"
                        : $"DELETE t FROM {stagingTableName} s JOIN {destinationTableName} t ON {joinCondition}";
        ...
        CancellationToken cancellationToken = default) where T : class
    {
        ...
        {
            // EF Core couldn't translate the value expression; fall back to raw SQL UPDATE.
            using var dbTransactionContext = new DbTransactionContext(dbContext, commandTimeout);
            try
            {
                int rowsAffected = await ExecuteRawUpdateFallbackAsync(queryable, updateExpression, dbContext, commandTimeout, cancellationToken);
                dbTransactionContext.Commit();
                return rowsAffected;
            }
            catch (Exception)
    ...
    private static async Task<int> ExecuteRawUpdateFallbackAsync<T>(IQueryable<T> queryable, Expression<Func<T, T>> updateExpression, DbContext dbContext, int? commandTimeout, CancellationToken cancellationToken) where T : class
    {
        var entityType = dbContext.Model.FindEntityType(typeof(T));
        var tableName = entityType.GetTableName();
        var tableNameDelimited = dbContext.DelimitIdentifier(tableName);

        string setClause = updateExpression.ToSqlUpdateSetExpression(dbContext, tableName);

        string querySql = queryable.ToQueryString();
        var parameters = ParseQueryStringParamsAsync(querySql);
        string alias = ExtractTableAliasAsync(querySql);
        string whereClause = ExtractWhereClauseAsync(querySql, alias);

        string updateSql = string.IsNullOrEmpty(whereClause)
            ? $"UPDATE {tableNameDelimited} SET {setClause}"
            : $"UPDATE {tableNameDelimited} SET {setClause} WHERE {whereClause}";

        return await dbContext.Database.ExecuteSqlAsync(updateSql, parameters?.ToArray(), commandTimeout, cancellationToken);
    }
    private static string ExtractTableAliasAsync(string querySql)
    {
        string sql = string.Join("\n", querySql.Split('\n').Where(l => !l.TrimStart().StartsWith("--")));
        int fromIdx = sql.IndexOf("FROM \"", StringComparison.Ordinal);
        if (fromIdx < 0) return null;
        int asIdx = sql.IndexOf("\" AS \"", fromIdx, StringComparison.Ordinal);
        if (asIdx < 0) return null;
        int aliasStart = asIdx + 6;
        int aliasEnd = sql.IndexOf("\"", aliasStart);
        if (aliasEnd < 0) return null;
        return sql.Substring(aliasStart, aliasEnd - aliasStart);
    }
    private static string ExtractWhereClauseAsync(string querySql, string alias)
    {
        string sql = string.Join("\n", querySql.Split('\n').Where(l => !l.TrimStart().StartsWith("--")));
        ...
        int orderByIdx = whereClause.IndexOf("\nORDER BY", StringComparison.OrdinalIgnoreCase);
        if (orderByIdx >= 0)
            whereClause = whereClause.Substring(0, orderByIdx).Trim();
        if (alias != null)
            whereClause = whereClause.Replace($"\"{alias}\".", "");
        return whereClause;
    }
    private static List<SqliteParameter> ParseQueryStringParamsAsync(string querySql)
    {
        var result = new List<SqliteParameter>();
        foreach (var line in querySql.Split('\n'))
        {
            if (!line.TrimStart().StartsWith("--")) continue;
            var match = Regex.Match(line, @"--\s+(@[\w]+)=(?:'((?:[^']|'')*)'|(NULL))");
            if (!match.Success) continue;
            string paramName = match.Groups[1].Value;
            bool isNull = match.Groups[3].Success;
            string paramValue = isNull ? null : match.Groups[2].Value.Replace("''", "'");
            result.Add(new SqliteParameter(paramName, (object)paramValue ?? DBNull.Value));
        }
    ...
    IEnumerable<string> inputColumns = null, bool useInternalId = false, CancellationToken cancellationToken = default)
{
    ...
    string delimColumnList = string.Join(",", columnNames.Select(tableMapping.DbContext.DelimitIdentifier));
    string paramList = string.Join(",", Enumerable.Range(0, columnNames.Count).Select(i => $"@p{i}"));
    string insertSql = $"INSERT INTO {tableName} ({delimColumnList}) VALUES ({paramList})";
    ...
    while (dataReader.Read())
    {
        for (int i = 0; i < columnNames.Count; i++)
        {
            object value = dataReader.GetValue(dataReader.GetOrdinal(columnNames[i]));
...
internal static async Task<BulkQueryResult> BulkQueryAsync(this DbContext context, string sqlText, DbConnection dbConnection, DbTransaction transaction, BulkOptions options, CancellationToken cancellationToken = default)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await using var command = dbConnection.CreateCommand();
        command.CommandText = DatabaseFacadeExtensions.RewriteDecimalComparisons(sqlText, parameters);
        if (parameters != null)
